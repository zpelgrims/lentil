...
int main(int argc, char *argv[])
{
  ...
  while(1)
  {
    ...
    Eigen::VectorXd ray_in(5); ray_in <<
      // p_rad * 4.0f * (x-0.5f),
      // p_rad * 4.0f * (y-0.5f),
  ...
  for(int i=0;i<valid;i++)
  {
    ...
    Eigen::VectorXd ray_in(5); ray_in << sample_in[5*i], sample_in[(5*i)+1], sample_in[(5*i)+2], sample_in[(5*i)+3], sample_in[(5*i)+4]; //not sure about this, could be wrong
    Eigen::VectorXd out(5); out.setZero();
