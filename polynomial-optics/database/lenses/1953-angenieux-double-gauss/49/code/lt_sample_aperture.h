...
if(1 || view(2) >= camera->lens_field_of_view)
{
  ...
  while(k<100&&(sqr_err>eps||sqr_ap_err>eps)&&error==0)
  {
    ...
    AiMsgInfo("pred_ap: %f %f", pred_ap(0), pred_ap(1));
    const Eigen::Vector2d delta_ap(ap_x - pred_ap[0], ap_y - pred_ap[1]);
    AiMsgInfo("delta_ap: %f %f", delta_ap(0), delta_ap(1));
    ...
    AiMsgInfo("dx1_domaga0: %f %f %f %f", dx1_domega0(0, 0), dx1_domega0(0, 1), dx1_domega0(1, 0), dx1_domega0(1, 1));
    ...
    AiMsgInfo("invApJ: %f %f %f %f", invApJ(0, 0), invApJ(1, 1), invApJ(0, 1), invApJ(1, 0));
    for(int i=0;i<2;i++)
    {
    ...
    AiMsgInfo("out: %f %f %f %f", out(0), out(1), out(2), out(3));
    ...
    AiMsgInfo("pred_out_cs_pos: %f %f %f", pred_out_cs_pos(0), pred_out_cs_pos(1), pred_out_cs_pos(2));
    AiMsgInfo("pred_out_cs_dir: %f %f %f", pred_out_cs_dir(0), pred_out_cs_dir(1), pred_out_cs_dir(2));
    Eigen::Vector3d view(
    ...
    AiMsgInfo("out_new_pos: %f %f", out_new_pos(0), out_new_pos(1));
    AiMsgInfo("out_new_dir: %f %f", out_new_dir(0), out_new_dir(1));
    out_new(0) = out_new_pos(0);
    out_new(1) = out_new_pos(1);
    out_new(2) = out_new_dir(0);
    out_new(3) = out_new_dir(1);
    const Eigen::Vector2d delta_out(out_new[2] - out[2], out_new[3] - out[3]);
    AiMsgInfo("delta_out: %f %f", delta_out(0), delta_out(1));
    ...
    AiMsgInfo("domega2_dx0: %f %f %f %f", domega2_dx0(0, 0), domega2_dx0(0, 1), domega2_dx0(1, 0), domega2_dx0(1, 1));
    Eigen::Matrix2d invJ;
    ...
    AiMsgInfo("invJ: %f %f %f %f", invJ(0, 0), invJ(1, 1), invJ(0, 1), invJ(1, 0));
    for(int i=0;i<2;i++)
    {
