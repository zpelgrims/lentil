...
if(1 || view(2) >= camera->lens_field_of_view)
{
  ...
  for(int k=0;k<100&&(sqr_err>eps||sqr_ap_err>eps)&&error==0;k++)
  {
    ...
    out(0) =  + 99.6253 *begin_dx + -0.612819 *begin_x + 0.591016 *begin_x*begin_lambda + -38.4082 *begin_dx*lens_ipow(begin_dy, 2) + -38.4111 *lens_ipow(begin_dx, 3) + 0.1689 *begin_y*begin_dx*begin_dy + -0.407523 *begin_x*lens_ipow(begin_lambda, 2) + 0.29064 *begin_x*lens_ipow(begin_dy, 2) + 0.460483 *begin_x*lens_ipow(begin_dx, 2) + 0.00558501 *begin_x*begin_y*begin_dy + -0.000247535 *begin_x*lens_ipow(begin_y, 2) + 0.00902979 *lens_ipow(begin_x, 2)*begin_dx + -0.000248469 *lens_ipow(begin_x, 3) + 0.00623469 *lens_ipow(begin_y, 2)*begin_dx*begin_lambda;
    out(1) =  + 99.6623 *begin_dy + -0.612999 *begin_y + 0.577961 *begin_y*begin_lambda + -38.5652 *lens_ipow(begin_dy, 3) + -38.4398 *lens_ipow(begin_dx, 2)*begin_dy + -0.383127 *begin_y*lens_ipow(begin_lambda, 2) + 0.458346 *begin_y*lens_ipow(begin_dy, 2) + 0.288674 *begin_y*lens_ipow(begin_dx, 2) + -0.000247733 *lens_ipow(begin_y, 3) + 0.165022 *begin_x*begin_dx*begin_dy + 0.0055095 *begin_x*begin_y*begin_dx + 0.00332464 *lens_ipow(begin_x, 2)*begin_dy + -0.000249142 *lens_ipow(begin_x, 2)*begin_y + 0.0161747 *lens_ipow(begin_y, 2)*begin_dy*begin_lambda;
    out(2) =  + -0.6051 *begin_dx + -0.00629425 *begin_x + -0.00359449 *begin_x*begin_lambda + 0.235336 *lens_ipow(begin_dx, 3) + 0.00254505 *begin_x*lens_ipow(begin_lambda, 2) + -0.00116404 *begin_x*lens_ipow(begin_dy, 2) + -2.17887e-05 *begin_x*begin_y*begin_dy + 1.68674e-06 *begin_x*lens_ipow(begin_y, 2) + -3.36134e-05 *lens_ipow(begin_x, 2)*begin_dx + 1.78267e-06 *lens_ipow(begin_x, 3) + 0.508465 *begin_dx*lens_ipow(begin_dy, 2)*begin_lambda + -0.397888 *begin_dx*lens_ipow(begin_dy, 2)*lens_ipow(begin_lambda, 2) + 0.000113728 *lens_ipow(begin_y, 2)*lens_ipow(begin_dx, 3)*begin_lambda + 0.000352478 *lens_ipow(begin_x, 2)*lens_ipow(begin_dx, 3)*begin_lambda;
    out(3) =  + -0.611319 *begin_dy + -0.00702857 *begin_y + 0.0105567 *begin_dy*begin_lambda + -0.000818453 *begin_y*begin_lambda + 0.238585 *lens_ipow(begin_dy, 3) + 0.336579 *lens_ipow(begin_dx, 2)*begin_dy + -3.0244e-05 *lens_ipow(begin_y, 2)*begin_dy + 1.74466e-06 *lens_ipow(begin_y, 3) + 0.00192569 *begin_x*begin_dx*begin_dy + -2.32218e-05 *begin_x*begin_y*begin_dx + 1.74817e-06 *lens_ipow(begin_x, 2)*begin_y + -0.0200531 *begin_y*lens_ipow(begin_dx, 2)*lens_ipow(begin_lambda, 2) + 0.00017072 *lens_ipow(begin_y, 2)*lens_ipow(begin_dy, 3) + 0.0241728 *begin_y*lens_ipow(begin_dx, 2)*lens_ipow(begin_lambda, 3);
    ...
    domega2_dx0(0, 0) =  + -0.00629425  + -0.00359449 *begin_lambda + 0.00254505 *lens_ipow(begin_lambda, 2) + -0.00116404 *lens_ipow(begin_dy, 2) + -2.17887e-05 *begin_y*begin_dy + 1.68674e-06 *lens_ipow(begin_y, 2) + -6.72269e-05 *begin_x*begin_dx + 5.348e-06 *lens_ipow(begin_x, 2) + 0.000704955 *begin_x*lens_ipow(begin_dx, 3)*begin_lambda+0.0f;
    domega2_dx0(0, 1) =  + -2.17887e-05 *begin_x*begin_dy + 3.37347e-06 *begin_x*begin_y + 0.000227456 *begin_y*lens_ipow(begin_dx, 3)*begin_lambda+0.0f;
    domega2_dx0(1, 0) =  + 0.00192569 *begin_dx*begin_dy + -2.32218e-05 *begin_y*begin_dx + 3.49634e-06 *begin_x*begin_y+0.0f;
    domega2_dx0(1, 1) =  + -0.00702857  + -0.000818453 *begin_lambda + -6.04881e-05 *begin_y*begin_dy + 5.23398e-06 *lens_ipow(begin_y, 2) + -2.32218e-05 *begin_x*begin_dx + 1.74817e-06 *lens_ipow(begin_x, 2) + -0.0200531 *lens_ipow(begin_dx, 2)*lens_ipow(begin_lambda, 2) + 0.00034144 *begin_y*lens_ipow(begin_dy, 3) + 0.0241728 *lens_ipow(begin_dx, 2)*lens_ipow(begin_lambda, 3)+0.0f;
    Eigen::Matrix2f invJ;
...
const float begin_lambda = lambda;
if(error==0)
  out[4] =  + 0.240858  + 0.845399 *begin_lambda + -1.1589 *lens_ipow(begin_lambda, 2) + 0.560249 *lens_ipow(begin_lambda, 3) + -0.250335 *lens_ipow(begin_dy, 4) + -0.46332 *lens_ipow(begin_dx, 2)*lens_ipow(begin_dy, 2) + -0.250803 *lens_ipow(begin_dx, 4) + -0.00502705 *begin_y*lens_ipow(begin_dy, 3) + -0.00400387 *begin_y*lens_ipow(begin_dx, 2)*begin_dy + -0.000122994 *lens_ipow(begin_y, 2)*lens_ipow(begin_dy, 2) + -0.00401476 *begin_x*begin_dx*lens_ipow(begin_dy, 2) + -0.00498555 *begin_x*lens_ipow(begin_dx, 3) + -0.000123777 *begin_x*begin_y*begin_dx*begin_dy + -0.000120986 *lens_ipow(begin_x, 2)*lens_ipow(begin_dx, 2);
else
  out[4] = 0.0f;
} break;
