...
if(1 || view(2) >= camera->lens_field_of_view)
{
  ...
  for(int k=0;k<100&&(sqr_err>eps||sqr_ap_err>eps)&&error==0;k++)
  {
    ...
    out(0) =  + 50.1459 *begin_dx + -0.511723 *begin_x + 0.189872 *begin_x*begin_lambda + -20.779 *begin_dx*lens_ipow(begin_dy, 2) + -20.3287 *lens_ipow(begin_dx, 3) + 0.252909 *begin_x*lens_ipow(begin_dy, 2) + 0.318411 *begin_x*lens_ipow(begin_dx, 2) + 0.00592114 *begin_x*begin_y*begin_dy + -0.0011197 *begin_x*lens_ipow(begin_y, 2) + 0.0132211 *lens_ipow(begin_x, 2)*begin_dx + -0.000831514 *lens_ipow(begin_x, 3) + 2.55569e-05 *lens_ipow(begin_y, 4)*begin_dx + -2.16711e-06 *lens_ipow(begin_x, 3)*lens_ipow(begin_y, 2) + -1.22874e-06 *lens_ipow(begin_x, 5);
    out(1) =  + 50.136 *begin_dy + -0.512954 *begin_y + 0.190038 *begin_y*begin_lambda + -20.2295 *lens_ipow(begin_dy, 3) + -20.7874 *lens_ipow(begin_dx, 2)*begin_dy + 0.329953 *begin_y*lens_ipow(begin_dy, 2) + 0.252932 *begin_y*lens_ipow(begin_dx, 2) + 0.013515 *lens_ipow(begin_y, 2)*begin_dy + -0.000819861 *lens_ipow(begin_y, 3) + 0.00582584 *begin_x*begin_y*begin_dx + -0.00111165 *lens_ipow(begin_x, 2)*begin_y + -1.25426e-06 *lens_ipow(begin_y, 5) + -2.26339e-06 *lens_ipow(begin_x, 2)*lens_ipow(begin_y, 3) + 2.52991e-05 *lens_ipow(begin_x, 4)*begin_dy;
    out(2) =  + -0.609054 *begin_dx + -0.0134161 *begin_x + -0.00247318 *begin_x*begin_lambda + 0.167404 *begin_dx*lens_ipow(begin_dy, 2) + 0.271939 *lens_ipow(begin_dx, 3) + -0.00243114 *begin_x*lens_ipow(begin_dy, 2) + -0.000100289 *begin_x*begin_y*begin_dy + 8.72631e-06 *begin_x*lens_ipow(begin_y, 2) + 1.06648e-05 *lens_ipow(begin_x, 3) + 2.22203e-08 *begin_x*lens_ipow(begin_y, 4) + 3.27548e-05 *lens_ipow(begin_x, 2)*begin_y*begin_dx*begin_dy + 4.59792e-08 *lens_ipow(begin_x, 3)*lens_ipow(begin_y, 2) + -5.39441e-07 *lens_ipow(begin_x, 4)*begin_dx + 1.65734e-08 *lens_ipow(begin_x, 5);
    out(3) =  + -0.612368 *begin_dy + -0.0133897 *begin_y + -0.00244101 *begin_y*begin_lambda + 0.27869 *lens_ipow(begin_dy, 3) + 0.368978 *lens_ipow(begin_dx, 2)*begin_dy + -0.00440674 *begin_y*lens_ipow(begin_dx, 2) + 9.8295e-06 *lens_ipow(begin_y, 3) + 0.00456728 *begin_x*begin_dx*begin_dy + -9.96712e-05 *begin_x*begin_y*begin_dx + 9.43077e-06 *lens_ipow(begin_x, 2)*begin_y + -5.31726e-07 *lens_ipow(begin_y, 4)*begin_dy + 1.82196e-08 *lens_ipow(begin_y, 5) + 4.80663e-08 *lens_ipow(begin_x, 2)*lens_ipow(begin_y, 3) + 2.37038e-08 *lens_ipow(begin_x, 4)*begin_y;
    ...
    domega2_dx0(1, 0) =  + 0.00456728 *begin_dx*begin_dy + -9.96712e-05 *begin_y*begin_dx + 1.88615e-05 *begin_x*begin_y + 9.61326e-08 *begin_x*lens_ipow(begin_y, 3) + 9.48153e-08 *lens_ipow(begin_x, 3)*begin_y+0.0f;
    domega2_dx0(1, 1) =  + -0.0133897  + -0.00244101 *begin_lambda + -0.00440674 *lens_ipow(begin_dx, 2) + 2.94885e-05 *lens_ipow(begin_y, 2) + -9.96712e-05 *begin_x*begin_dx + 9.43077e-06 *lens_ipow(begin_x, 2) + -2.1269e-06 *lens_ipow(begin_y, 3)*begin_dy + 9.10981e-08 *lens_ipow(begin_y, 4) + 1.44199e-07 *lens_ipow(begin_x, 2)*lens_ipow(begin_y, 2) + 2.37038e-08 *lens_ipow(begin_x, 4)+0.0f;
    Eigen::Matrix2f invJ;
