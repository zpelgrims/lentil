...
if [ "$OS" == "windows" ]; then
   sshpass -p "Fv5[E)@y6MYHYCyY" ssh $BUILD_SERVER "cd C:/lentil-build/lentil/pota/build/server && build_server_windows.bat $LENSES $USER $USER_BUILD_FOLDER $DOWNLOAD_DIR"
elif [ "$OS" == "linux" ]; then
